...
void listTypePush(robj *subject, robj *value, int where) {
    ...
    if (subject->encoding == OBJ_ENCODING_QUICKLIST) {
        ...
        value = getDecodedObject(value);            // 从value中解码出数据项
        size_t len = sdslen(value->ptr);            // 得到数据项的长度

        // 调用quicklistPush插入数据
        quicklistPush(subject->ptr, value->ptr, len, pos);
        decrRefCount(value);                        // 将数据项对象的引用次数减1，也就是释放value
    } else {
...
void pushxGenericCommand(client *c, int where) {
    ...
    // 添加数据元素
    for (j = 2; j < c->argc; j++) {
        listTypePush(subject,c->argv[j],where);     // 将元素添加到列表中
        pushed++;                                   // 记录添加的元素个数
    }
...
void blockForKeys(client *c, robj **keys, int numkeys, mstime_t timeout, robj *target) {
    ...
    c->bpop.timeout = timeout;                      // 设定阻塞超时时间
    c->bpop.target = target;                        // 设置目标选项，target在执行RPOPLPUSH命令时使用
    ...
    for (j = 0; j < numkeys; j++) {
        ...
        if (de == NULL) {
            ...
            l = listCreate();   // 创建新的list
            retval = dictAdd(c->db->blocking_keys,keys[j],l);   // 将键和值加入到c->db->blocking_keys中
            incrRefCount(keys[j]);
            serverAssertWithInfo(c,keys[j],retval == DICT_OK);
        } else {
            l = dictGetVal(de);
        }
        listAddNodeTail(l,c);       // 将客户端加入到链表中
    }
    blockClient(c,BLOCKED_LIST);    // 阻塞该客户端
}
...
void unblockClientWaitingData(client *c) {
    ...
    // 获取一个字典迭代器，迭代造成客户端阻塞的key
    di = dictGetIterator(c->bpop.keys);

    // 遍历所有阻塞的key---这里造成客户端阻塞的key不是只有一个么？？？？
    while((de = dictNext(di)) != NULL) {
        ...
        // 如果客户端链表为空，删除db->blocking_keys中的key
        if (listLength(l) == 0)
...
void handleClientsBlockedOnLists(void) {
    while(listLength(server.ready_keys) != 0) {
        ...
        while(listLength(l) != 0) {
            listNode *ln = listFirst(l);                // 取出server.ready_keys中的第一个节点
            readyList *rl = ln->value;                  // 指向redislist结构

            dictDelete(rl->db->ready_keys,rl->key);     // 从ready_keys中移除就绪的key
            ...
            if (o != NULL && o->type == OBJ_LIST) {
                ...
                if (de) {
                    ...
                    while(numclients--) {
                        listNode *clientnode = listFirst(clients);                  // 取出客户端
                        client *receiver = clientnode->value;
                        robj *dstkey = receiver->bpop.target;                       // 设置弹出的目标对象（只在 BRPOPLPUSH 时使用）
