...
zskiplist *zslCreate(void) {
    ...
    zsl->header->backward = NULL;                                      // 设定backward指向NULL
    zsl->tail = NULL;
    return zsl;
}
...
zskiplistNode *zslInsert(zskiplist *zsl, double score, sds ele) {
    ...
    if (level > zsl->level) {
        ...
        zsl->level = level;                                     // 更新level值
    }
    x = zslCreateNode(level,score,ele);                         // 根据随机生成一个level值、分数、ele创建一个节点
    for (i = 0; i < level; i++) {
        x->level[i].forward = update[i]->level[i].forward;      // 针对跳跃表的每一层，改变其forward指针的指向
        update[i]->level[i].forward = x;

        x->level[i].span = update[i]->level[i].span - (rank[0] - rank[i]);// 更新插入节点的span值
        update[i]->level[i].span = (rank[0] - rank[i]) + 1;               // 更新插入点的前一个节点的span值
    }
...
void zslDeleteNode(zskiplist *zsl, zskiplistNode *x, zskiplistNode **update) {
    ...
    for (i = 0; i < zsl->level; i++) {
        if (update[i]->level[i].forward == x) {// 如果x存在于该层，则需要修改前一个节点的前向指针
...
//根据节点的分值和成员来删除该节点，其他两个操作无非是在查找节点上有区别
int zslDelete(zskiplist *zsl, double score, sds ele, zskiplistNode **node) {
    ...
    if (x && score == x->score && sdscmp(x->ele,ele) == 0) {
        zslDeleteNode(zsl, x, update);// 调用底层删除节点函数
        if (!node)
            zslFreeNode(x);
        else
            *node = x;
...
unsigned long zslDeleteRangeByScore(zskiplist *zsl, zrangespec *range, dict *dict) {
    ...
    {
        zskiplistNode *next = x->level[0].forward;           // 保存下一个节点
        zslDeleteNode(zsl,x,update);                         // 删除该节点
        dictDelete(dict,x->ele);                             // 删除该节点的成员
            zslFreeNode(x);                                  // 释放该节点
        removed++;
        x = next;
    }
