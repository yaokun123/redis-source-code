...
struct redisCommand redisCommandTable[] = {
    ...
    {"debug2",debugCommand2,-1,"as",0,NULL,0,0,0,0,0},
};
...
int serverCron(struct aeEventLoop *eventLoop, long long id, void *clientData) {
    ...
    // 更新缓存当前时间server.unixtime和server.mstime
    updateCachedTime();
    ...
    // 在启动服务器时，redis会为服务器进程的信号关联处理器sigShutdownHandler
    if (server.shutdown_asap) {
        if (prepareForShutdown(SHUTDOWN_NOFLAGS) == C_OK) exit(0);
        serverLog(LL_WARNING,"SIGTERM received but errors trying to shut down the server, check the logs for more information");
        server.shutdown_asap = 0;   // 关闭，关闭标识
    }
        ...
        ldbPendingChildren())
    {
        ...
        //// 循环判断rdb的 save条件，查看是否满足执行bgsave条件
...
int processCommand(client *c) {
    ...
    // 从命令字典中找到命令字符串对应的redisCommand结构
